...
TILED_PANEL_WIDTH = 238.0
TILED_PANEL_HEIGHT = 126.0
TILED_PANEL_GAP_X = 12.0
TILED_PANEL_GAP_Y = 7.0
TILED_PANEL_TOP_LEFT = (
    (TEXTURE_SIZE[0] - (TILED_PANEL_WIDTH * COLUMNS + TILED_PANEL_GAP_X * (COLUMNS - 1))) * 0.5,
    198.0,
)
TILE_SPRITE_MARGIN = 52
PANEL_DEPTH_PIXELS = 30
PANEL_DEPTH_SHEAR_PIXELS = 0
...
def create_board_frame(corners_by_cell):
    ...
    top = min(ys) - 10
    right = max(xs) + 12
    bottom = max(ys) + PANEL_DEPTH_PIXELS + 24
    draw = ImageDraw.Draw(frame)

    shadow = Image.new("RGBA", TEXTURE_SIZE, (0, 0, 0, 0))
    shadow_draw = ImageDraw.Draw(shadow)
    shadow_draw.rounded_rectangle((left, top + 10, right, bottom + 14), radius=10, fill=(0, 0, 0, 118))
    shadow = shadow.filter(ImageFilter.GaussianBlur(8))
    frame.alpha_composite(shadow)

    draw.rounded_rectangle((left, max(top + 22, bottom - 42), right, bottom), radius=8, fill=(2, 7, 11, 210), outline=(18, 34, 44, 155), width=3)
    draw.rounded_rectangle((left, top, right, bottom - 20), radius=8, fill=(5, 12, 18, 70), outline=(45, 68, 78, 95), width=2)
    split_x = (corners_by_cell[(0, 2)][1][0] + corners_by_cell[(0, 3)][0][0]) * 0.5
    draw.line((split_x, top + 4, split_x, bottom - 8), fill=(38, 82, 116, 185), width=4)
    ...
    rail_y = bottom - 22
    draw.line((ally_left, rail_y, ally_right, rail_y), fill=(255, 54, 28, 225), width=7)
    draw.line((enemy_left, rail_y, enemy_right, rail_y), fill=(36, 184, 255, 225), width=7)
    draw.line((ally_left, rail_y + 12, ally_right, rail_y + 12), fill=(255, 54, 28, 118), width=4)
    draw.line((enemy_left, rail_y + 12, enemy_right, rail_y + 12), fill=(36, 184, 255, 118), width=4)
    draw.line((ally_left, top + 8, ally_right, top + 8), fill=(255, 54, 28, 72), width=2)
    draw.line((enemy_left, top + 8, enemy_right, top + 8), fill=(36, 184, 255, 72), width=2)
